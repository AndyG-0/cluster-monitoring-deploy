    ...
    stages {
        ...
        stage('Deploy to k3s') {
            ...
            steps {
                    ...
                    sh 'ls -l'
                    sh 'export KUBECONFIG=/var/lib/jenkins/config && cd ./cluster-monitoring/ && kubectl apply -f ./manifests/setup/ && kubectl apply -f ./manifests/ && sleep 5 && kubectl apply -f ./manifests/'
            }
        }
    }
    post {
