pipeline {
    ...
    stages {
        ...
        stage('Execute Make') {
            steps {
                script {
                    sh 'cd ./cluster-monitoring/ &&  export GO111MODULE=on && go get github.com/jsonnet-bundler/jsonnet-bundler/cmd/jb@latest && go get github.com/google/go-jsonnet/cmd/jsonnet@latest && make vendor && make'
                }
            }
        }
        stage('Deploy to k3s') {
